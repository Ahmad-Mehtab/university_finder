...
if(isset($_POST['search']))
{
	...
	$deptid = $_POST['deptid'];
	$fees = $_POST['fees'];	
	$ratingqry = "SELECT * FROM universities WHERE total_rating <= '".$ratings."'";
	$runratingqry = mysql_query($ratingqry);
...
<div class="row">
<div class="col-sm-12">
<h3 class="text-center text-light">Your Search Result.</h3>
<p class="text-center text-light">Find Out & Explore best Univeristies near your area and see what's happening !!!</p>
</div>
</div>
<br><br>
...
while($getunivbyrating=mysql_fetch_array($runratingqry))
{
	
$getunivid = $getunivbyrating['id'];

$getsumqry = "SELECT SUM(total) as totalfees FROM fee_structure WHERE dept_id = '".$deptid."' AND univ_id = '".$getunivid."'  GROUP BY univ_id";
$sumqryrun = mysql_query($getsumqry);
$fetchsumqry = mysql_fetch_array($sumqryrun);
$getquerycount = mysql_num_rows($sumqryrun);
$totalfees = $fetchsumqry['totalfees'];
...
{
	echo "yes";
if(($totalfees <= $fees) && ($getquerycount != 0) && ($totalfees > $fees))
{	
?>
<div class="col-sm-3">
<div class="card">
	<div class="card-body">
	 <a href="cecos/index.php?uid=<?php echo $getunivbyrating['id'];?>">
     <img style="width:200px; height:200px;" src="univ_img/<?php echo $getunivbyrating['university_logo'];?>" class="img-fluid"></a>
	</div>
	<div class="card-footer bg-dark">
		<h6 class="text-light"><?php echo $getunivbyrating['university_name'];?></h6>
	</div>
</div>
</div>
<?php
